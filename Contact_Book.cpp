...
void SearchContact(char FullName[][SIZE_NAME], char CountryCode[][SIZE_COUNTRY_CODE], char PhoneNumber[][SIZE_PHONE_NUMBER], char Email[][SIZE_EMAIL], int &Maxindex);
    ...
    */
    while(operation != 5) {
        ...
        case 3:
            ...
            break;
        case 4: 
            SearchContact(FullName, CountryCode, PhoneNumber, Email, NumberOfContact);
            break;
        }

        operation = ContactTable(); // update the input choice.
    }
    if(operation == 5) {
            cout << "Exiting......" << endl;
    }
...
void DelletContact(char FullName[][SIZE_NAME], char CountryCode[][SIZE_COUNTRY_CODE], char PhoneNumber[][SIZE_PHONE_NUMBER], char Email[][SIZE_EMAIL], int &Maxindex){
    ...
    // accepting the name from the user to be Deleted.
...
void SearchContact(char FullName[][SIZE_NAME], char CountryCode[][SIZE_COUNTRY_CODE], char PhoneNumber[][SIZE_PHONE_NUMBER], char Email[][SIZE_EMAIL], int &Maxindex){
    int i, count = 0;
    // accepting the name from the user which to be searched.
    bool found = false;
    char SearchName[SIZE_NAME];
    cout << "Enter The name You want to Search: ";
    cin.ignore();
    cin.getline(SearchName, SIZE_NAME - 1);
    ...
    for (i = 0; i < Maxindex; i++) {
        if (strcmp(SearchName, FullName[i]) == 0) { // if the name to delete is find in the indexes.
            count++;
            found = true;
            break; // once got the name should be stop the itration.
        }
    ...
    if (found) {
        ...
        cout << SearchName << " NO SUCH NAME IN CONTACT\n";
    }
}
